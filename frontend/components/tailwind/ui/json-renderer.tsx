...
export function JsonRenderer({ content, className = "" }: JsonRendererProps) {
  const { isSowJson, formattedContent } = useMemo(() => {
    try {
      ...
      const isSowData =
        parsed.scopeItems || parsed.pricing || parsed.markdownContent || // Original format
        parsed.scopes || parsed.currency || // New pricing format
        parsed.scope_name || parsed.scope_description; // Scope format with asterisks
      ...
      return {
        isSowJson: false,
        formattedContent: (
          <pre className={`${className} text-xs font-mono whitespace-pre-wrap break-all overflow-hidden bg-[#0a0a0a] p-3 rounded border border-[#1b5e5e]`}>
  ...
  const normalizeData = (raw: any) => {
    ...
    if (raw.scopes && Array.isArray(raw.scopes)) {
      const normalizedScopes = raw.scopes.map((scope: any) => ({
        ...
        roles: getValue(scope, ['roles', 'role_allocation', 'role*allocation']) || []
      }));
      ...
      return {
        ...
        pricing: {
          ...
          grand_total_pre_gst: getValue(raw, ['grand_total_pre_gst', 'grand*total*pre*gst']),
    ...
    if (raw.scope_name || raw.scope*name) {
      return {
        scopeItems: [{
          scope_name: getValue(raw, ['scope_name', 'scope*name']),
          scope_total: getValue(raw, ['scope_total', 'scope*total']),
          roles: getValue(raw, ['roles', 'role_allocation', 'role*allocation']) || []
        }],
        pricing: {
          currency: getValue(raw, ['currency']),
          gst_rate: getValue(raw, ['gst_rate', 'gst*rate']),
            ...
            <table className="min-w-full text-xs border border-[#0E2E33] max-w-full table-fixed">
              <thead>
                <tr className="bg-[#0E2E33]">
                  <th className="p-2 text-left text-white w-1/3">Scope</th>
                  <th className="p-2 text-left text-white w-1/4">Total</th>
                  <th className="p-2 text-left text-white w-5/12">Roles</th>
                </tr>
              </thead>
              <tbody>
                {normalizedData.scopeItems.map((scope: any, idx: number) => (
                  <tr key={idx} className="border-t border-[#0E2E33]">
                    <td className="p-2 text-gray-300 break-words overflow-wrap-anywhere">{scope.scope_name || 'N/A'}</td>
                    <td className="p-2 text-gray-300 break-words overflow-wrap-anywhere">
                      {scope.scope_total ? `$${scope.scope_total.toLocaleString()}` : 'N/A'}
                    </td>
                    <td className="p-2 text-gray-300">
                      {Array.isArray(scope.roles) && scope.roles.length > 0 ? scope.roles.map((role: any, roleIdx: number) => (
                        <div key={roleIdx} className="text-xs break-words overflow-wrap-anywhere">
                          <div className="font-medium">{role.role}</div>
                          <div className="text-gray-400">
                            {role.hours}h @ ${role.rate}/hr = ${role.cost?.toLocaleString()}
                          </div>
                        </div>
                      )) : <div className="text-xs text-gray-500">No role allocation</div>}
                    </td>
                  </tr>
                ))}
              </tbody>
              ...
              <div className="space-y-1 text-xs">
                {normalizedData.pricing.grand_total_pre_gst && (
                  <div className="flex justify-between text-gray-300">
                    <span>Subtotal:</span>
                    <span>${normalizedData.pricing.grand_total_pre_gst.toLocaleString()}</span>
                  </div>
                )}
                {normalizedData.pricing.gst_rate && normalizedData.pricing.gst_amount && (
                  <div className="flex justify-between text-gray-300">
                    <span>GST ({normalizedData.pricing.gst_rate}%):</span>
                    <span>${normalizedData.pricing.gst_amount.toLocaleString()}</span>
                  </div>
                )}
                {normalizedData.pricing.grand_total && (
                  <div className="flex justify-between text-white font-semibold border-t border-[#1b5e5e] pt-1">
                    <span>Total ({normalizedData.pricing.currency || 'AUD'}):</span>
                    <span>${normalizedData.pricing.grand_total.toLocaleString()}</span>
                  </div>
                )}
              </div>
            </div>
