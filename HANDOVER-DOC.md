...
### **Making Changes**
1. **ONLY work in `/root/the11-dev-clean`** - This is the ONLY repository used for deployment
2. Make changes and test locally
3. Commit with descriptive messages
4. **Automatic Branch Detection for Push:**
   - **Frontend changes** (`frontend/*` files) → Push to `sow-latest` branch
   - **Backend changes** (`backend/*` files) → Push to `backend-service` branch
   - If both frontend and backend are changed, push separately or manually specify branch
5. Easypanel auto-deploys from `sow-latest` (frontend) and `backend-service` (backend)

### **Push Workflow (CRITICAL)**
- **Repository:** `https://github.com/khaledbashir/the11-dev-clean.git` (ONLY)
- **Frontend changes:** Always push to `sow-latest` branch
- **Backend changes:** Always push to `backend-service` branch
- **Never push to any other repository** - All work must be in `the11-dev-clean`

### **Backup Strategy**
- Original repo `/root/the11-dev` kept as backup (DO NOT USE FOR DEPLOYMENT)
- All branches preserved in original repo
- Clean repo has only 2 branches: `sow-latest` and `backend-service`

### **Branch Strategy**
- **`sow-latest`** - Frontend production branch (default, auto-deployed)
- **`backend-service`** - Backend-specific changes (auto-deployed)
